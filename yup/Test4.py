...
class Player(Object):
    ...
    def DrawBurger(self, surface, alpha=255):
        # Load character image based on selection
        character_img = pygame.image.load(CHARACTERS[game_data['selected_character']]["preview"]).convert_alpha()
        character_img = pygame.transform.scale(character_img, (self.width, self.height))
        temp_image = character_img.copy()
...
ITEM_PRICES = {
    "Galaxy": 50, "Neon": 100, "Gold": 200, "Regenboog": 500,
...
def show_start_menu(screen, font, big_font, game_data):
    ...
    while True:
        ...
        # Tekenen - animated background
        time_now = pygame.time.get_ticks() / 1000
        color_shift = (math.sin(time_now) + 1) / 2
        base_top = (60, 90, 160)
        base_bottom = (100, 140, 200)
        alt_top = (120, 60, 160)
        alt_bottom = (160, 100, 200)
        top_color = tuple(int(base_top[i] * (1 - color_shift) + alt_top[i] * color_shift) for i in range(3))
        bottom_color = tuple(int(base_bottom[i] * (1 - color_shift) + alt_bottom[i] * color_shift) for i in range(3))
...
def show_shop(screen, font, big_font, game_data):
    ...
    max_scroll = 0
    animation_time = 0
    selected_category = "trash"  # "trash" or "obstacles" or "character"
    ...
    def draw_item_card(surface, x, y, width, height, item_data, hovered=False):
        ...
        if owned:
            ...
            if selected:
                ...
                draw_professional_button(surface, deselect_btn_rect, "RESET", small_font, "warning")
            else:
                use_btn_rect = pygame.Rect(x + width - 180, y + height - 45, 80, 30)
                draw_professional_button(surface, use_btn_rect, "GEBRUIK", small_font, "success")
        else:
    ...
    def calculate_max_scroll():
        ...
        items_count = len(TRASH_COLORS) + len(OBSTACLE_COLORS) + len(CHARACTERS) - 3  # -3 for "Standaard" items
        cards_per_row = 2
    ...
    while True:
                ...
                for name, data in current_items.items():
                    ...
                    if y > -card_height and y < WINDOW_HEIGHT:  # Only check visible cards
                        ...
                        if card_rect.collidepoint(mouse_pos):
                            if owned:
                                if not selected:
                                    game_data[selected_key] = name  # Use item
                                else:
                                    game_data[selected_key] = "Standaard"  # Deselect
                                save_game_data(game_data)
                            else:
                                if can_afford:
                                    game_data[owned_key].append(name)  # Buy item
        ...
        character_text = subtitle_font.render("CHARACTER", True, TEXT_PRIMARY)
        ...
        screen.blit(character_text, character_text.get_rect(center=character_tab.rect.center))
