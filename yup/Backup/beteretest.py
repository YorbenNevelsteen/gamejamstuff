...
import json
import os
from AnimateGif import Animation
import math
...
def show_shop(SCREEN, font, big_font, game_data):
    ...
    while True:
        ...
        base_bottom = (100, 140, 200)  # lichtblauw
        alt_top = (120, 60, 160)  # paars
        alt_bottom = (160, 100, 200)  # lila

        # Interpoleer kleuren afhankelijk van tijd
        top_color = tuple(int(base_top[i] * (1 - color_shift) + alt_top[i] * color_shift) for i in range(3))
        bottom_color = tuple(int(base_bottom[i] * (1 - color_shift) + alt_bottom[i] * color_shift) for i in range(3))
        ...
        # wallet_bg = pygame.Rect(WINDOW_WIDTH // 2 - 120, 75, 240, 35)
        # pygame.draw.rect(SCREEN, SUCCESS_COLOR, wallet_bg, border_radius=20)
        # wallet_text = f"💰 {game_data['total_trash']} Afval"
        # wallet_surf = body_font.render(wallet_text, True, (255, 255, 255))
        # wallet_text_rect = wallet_surf.get_rect(center=wallet_bg.center)
        # SCREEN.blit(wallet_surf, wallet_text_rect)
